...
SELECT count(*)
FROM people
WHERE CURRENT_DATE - hire_date > 180

--Quantas pessoas PM’s estão a menos de 1 ano na Creditas

SELECT count (CURRENT_DATE - hire_date > 360) AS quantidade_dias
FROM people
...
SELECT people.name ,length(name) as tamanho from people
...
select tribes.name ,count (distinct people.name) , AVG(CURRENT_DATE - hire_date)/30  AS quantidade_dias from people
inner join  people_squad
	on people.id = people_squad.people_id
inner join squads 
	on people_squad.squad_id = squads.id
